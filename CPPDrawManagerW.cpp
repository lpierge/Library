...
CPPDrawManagerW::CPPDrawManagerW()
{
...
void CPPDrawManagerW::GetSizeOfIcon(HICON hIcon, LPSIZE pSize) const
{
	pSize->cx = 0;
	pSize->cy = 0;
	if (hIcon != NULL)
...
void CPPDrawManagerW::GetSizeOfBitmap(HBITMAP hBitmap, LPSIZE pSize) const
{
	pSize->cx = 0;
	pSize->cy = 0;
	if (hBitmap != NULL)
...
void CPPDrawManagerW::AlphaBitBlt(HDC hDestDC, int nDestX, int nDestY, DWORD dwWidth, DWORD dwHeight, HDC hSrcDC, int nSrcX, int nSrcY, int percent /* = 100 */)
{
...
void CPPDrawManagerW::AlphaChannelBitBlt(HDC hDestDC, int nDestX, int nDestY, DWORD dwWidth, DWORD dwHeight, HDC hSrcDC, int nSrcX, int nSrcY)
{
	_ASSERT ((NULL != hDestDC) || (NULL != hSrcDC));
...
HBITMAP CPPDrawManagerW::CreateImageEffect(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, DWORD dwEffect, BOOL bUseMask /* = TRUE */, COLORREF clrMask /* = RGB(255, 0, 255) */, COLORREF clrMono /* = RGB(255, 255, 255) */)
{
...
//----------------------------------------------------------
COLORREF CPPDrawManagerW::GrayMirrorColor(COLORREF clrColor)
{
...
COLORREF CPPDrawManagerW::GrayColor(COLORREF clrColor)
{
...
COLORREF CPPDrawManagerW::InvertColor(COLORREF clrColor)
{
	return RGB(255 - GetRValue(clrColor), 255 - GetGValue(clrColor), 255 - GetBValue(clrColor));
} //End InvertColor

COLORREF CPPDrawManagerW::DarkenColor(COLORREF clrColor, double darken)
{
...
COLORREF CPPDrawManagerW::LightenColor(COLORREF clrColor, double lighten)
{
...
COLORREF CPPDrawManagerW::PixelAlpha(COLORREF clrSrc, double src_darken, COLORREF clrDest, double dest_darken)
{
...
HICON CPPDrawManagerW::StretchIcon(HICON hIcon, DWORD dwWidth, DWORD dwHeight)
{
...
//LPI	ICONINFO csOriginal, csStretched;
	ICONINFO csOriginal = {0}, csStretched = {0};
	...
	if (::GetObjectW(csOriginal.hbmColor, sizeof(BITMAP), &bmp))
	{
		...
		if (NULL != csStretched.hbmColor)
		{
			...
			if (::GetObjectW(csOriginal.hbmMask, sizeof(BITMAP), &bmp))
			{
...
void CPPDrawManagerW::FillGradient (HDC hDC, LPCRECT lpRect, 
...
#ifdef USE_SHADE
void CPPDrawManagerW::SetShade(LPCRECT lpRect, UINT shadeID /* = 0 */, BYTE granularity /* = 8 */, 
...
void CPPDrawManagerW::FillEffect(HDC hDC, DWORD dwEffect, LPCRECT lpRect, COLORREF clrBegin, COLORREF clrMid /* = 0 */, COLORREF clrEnd /* = 0 */,  BYTE granularity /* = 0 */, BYTE coloring /* = 0 */)
{
	HBRUSH hBrush = NULL;
...
void CPPDrawManagerW::MultipleCopy(HDC hDestDC, int nDestX, int nDestY, DWORD dwDestWidth, DWORD dwDestHeight, 
...
void CPPDrawManagerW::DrawBitmap(HDC hDC, int x, int y, DWORD dwWidth, DWORD dwHeight, HBITMAP hSrcBitmap,
...
void CPPDrawManagerW::DrawIcon(HDC hDC, int x, int y, DWORD dwWidth, DWORD dwHeight, HICON hSrcIcon,
  							DWORD dwEffect /* = IMAGE_EFFECT_NONE */, 
							BOOL bShadow /* = FALSE */, 
							DWORD dwCxShadow /* = PPDRAWMANAGER_SHADOW_XOFFSET */, 
							DWORD dwCyShadow /* = PPDRAWMANAGER_SHADOW_YOFFSET */,
...
void CPPDrawManagerW::DrawShadow(HDC hDestDC, int nDestX, int nDestY, DWORD dwWidth, DWORD dwHeight, HBITMAP hMask, BOOL bGradient /* = FALSE */, DWORD dwDepthX /* = PPDRAWMANAGER_SHADOW_YOFFSET */, DWORD dwDepthY /* = PPDRAWMANAGER_SHADOW_XOFFSET */)
{
...
void CPPDrawManagerW::DrawImageList(HDC hDC, int x, int y, DWORD dwWidth, DWORD dwHeight, HBITMAP hSrcBitmap,
...
void CPPDrawManagerW::MaskToDepth(HDC hDC, DWORD dwWidth, DWORD dwHeight, HBITMAP hMask, double * pDepth, BOOL bGradient /* = FALSE */, DWORD dwDepthX /* = PPDRAWMANAGER_CXSHADOW */, DWORD dwDepthY /* = PPDRAWMANAGER_CYSHADOW */)
{
	HDC hSrcDC = ::CreateCompatibleDC(hDC);
	if (NULL == hSrcDC)
	{
...
void CPPDrawManagerW::DarkenByDepth(HDC hDC, int x, int y, DWORD dwWidth, DWORD dwHeight, double * pDepth)
{
	HDC hSrcDC = ::CreateCompatibleDC(hDC);
	if (NULL == hSrcDC)
	{
...
void CPPDrawManagerW::SmoothMaskImage(const int ImageWidth, 
...
void CPPDrawManagerW::GetPartialSums(const double* const pM,
									unsigned int nMRows,
									unsigned int nMCols,
...
//LPI	const unsigned int nResMRows = nMRows - nPartRows + 1;
...
void CPPDrawManagerW::DrawRectangle(HDC hDC, LPRECT lpRect, COLORREF crLight, COLORREF crDark, int nStyle /* = PEN_SOLID */, int nSize /* = 1 */)
{
	DrawRectangle(hDC, lpRect->left, lpRect->top, lpRect->right, lpRect->bottom, crLight, crDark, nStyle, nSize);
}

void CPPDrawManagerW::DrawRectangle(HDC hDC, int left, int top, int right, int bottom, 
...
void CPPDrawManagerW::DrawLine(HDC hDC, 
