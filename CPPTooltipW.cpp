/*
	...
	Il codice originale (CPPToolTip.cpp) era scritto usando le varie macro LPCTSTR, TCHAR, etc., il che 
	permetteva la compilazione in modalita' ANSI o Unicode, a seconda della configurazione del progetto.

	Mentre l'SDK di Windows contiene le versioni 'A' (ANSI) e 'W' (Wide) delle varie API, MFC solo provvede
	un unica interfaccia, con lo stesso nome funzione per ambe versioni (ANSI e Wide), dove cio' che varia
	e' la libreria usata per la compilazione a seconda di che macro venga definita (#ifdef _MBCS/_UNICODE).
	(SDK Windows: una libreria con due funzioni, MFC: due librerie con la stessa funzione)

	La prima modifica (2005) solo tocca punti specifici del codice per risolvere problemi minori, mentre la
	seconda modifica (2026) trasforma il codice con supporto Unicode esplicito, dato che deve essere usato
	in progetti ANSI che pero' devono poter gestire anche stringhe Unicode (in modo esplicito).
	Lasciando infatti il tipo originale LPCTSTR (in un progetto ANSI), questi viene tradotto a LPCSTR, non a
	LPCWSTR, generando quindi errore con l'uso in funzioni di tipo 'W', che si aspettano tipi Wide (Unicode).
	Questo non significa che il codice non possa essere usato in progetti Unicode nativi, ma che esplicita il
	supporto Wide per poterlo usare anche in progetti ANSI nativi.

	Qui, come nella versione ibrida originale (CPPToolTip.cpp), vengono gestiti i tre tipi di avvisi visuali:
	il tooltip, ed i due balloons, normale ed esteso.
	Nella versione originale, del 2004, il balloon era di due tipi, il normale, supportato in forma nativa da
	Windows, e l'esteso, simile al nativo ma con possibilita' di essere customizzato.
	Nelle Versioni moderne di Windows (a partire dalla 10?), il balloon nativo, con estetica tipo fumetti, e'
	stato sostitito dall'orribile e tristissima notificazione woke con il rettangolone nero, per cui il modo
	esteso e' l'unico a conservare l'estetica da fumetto.

	Note modifiche:

	su CPPToolTip.cpp (originale)
	- aggiunto: headers (#include)
	- aggiunto: TRACEEXPR
	- cambiato:	BOOL CPPToolTip::Create(CWnd* pParentWnd, BOOL bBalloon) in BOOL CPPToolTip::Create(HWND hWndParent, BOOL bBalloon)
	- commentate alcune linee di codice random

	Luca Piergentili, 2005

	su CPPToolTip.cpp modificato (vedi sopra) a CPPToolTipW.cpp
...
{
	wchar_t buf[16];
	GetClassNameW(hwnd, buf, sizeof(buf) / sizeof(wchar_t));
...
//	EnableTextWrap(FALSE);
	...
	if(!(::GetClassInfoW(hInst, PPTOOLTIP_CLASSNAME, &wndcls)))
	{
		...
		wndcls.hIcon			= NULL;
		wndcls.hCursor			= LoadCursorW(hInst, (LPCWSTR)IDC_ARROW);
...
BEGIN_MESSAGE_MAP(CPPToolTipW, CWnd)
...
//LPI
// 
// Note specifiche per Unicode esplicito:
// --------------------------------------
// questo blocco di codice a seguire e' fondamentale per la transizione da ANSI a Unicode esplicito ed il
// corretto funzionamento del balloon esteso
// tenere anche ben presente che tutto cio' avviene perche' il progetto e' nativamente ANSI, NON Unicode,
// ed usa quindi Unicode "al volo", solo quando lo necessita, se il progetto fosse invece Unicode nativo,
// il codice con la CreateEx() e le Afx...() non avrebbe necessitato nessun cambio
// questo perche' mentre Windows fornisce a livello di SDK due funzioni diverse da poter usare a seconda
// della necessita' (le 'A' e le 'W'), il framework MFC si basa su una compilazione condizionale, come
// #ifdef _UNICODE (o _MBCS per ANSI), per cui la modalita' (ANSI/Wide) va selezionata al principio nelle
// opzioni del progetto, per tirarsi dentro la versione corretta di MFC
// 
// la CWnd::CreateEx() (versione ANSI di MFC) si aspetta LPCSTR (8 bit) per il nome classe, ma ora, con il
// passaggio da ANSI a Unicode, la costante PPTOOLTIP_CLASSNAME diventa un LPCWSTR (16 bit)
// bisogna quindi scavalcare il metodo CreateEx() di MFC (vincolato al modo ANSI del progetto) e chiamare
// direttamente la funzione API nativa di Windows ::CreateWindowExW() per creare una finestra Unicode e
// passarla poi a MFC
// 
// pero' ::CreateWindowExW() bypassa l'hook di MFC, grazie al quale AfxWndProc() prende i messaggi di Windows
// WM_PAINT, WM_TIMER, WM_SHOWWINDOW, etc. e li smista ai metodi della classe (OnPaint, OnTimer, etc.) via
// Message Map, dato che usa come procedura la ::DefWindowProcW() registrata nella WNDCLASSW, il che fa si'
// che la finestra del balloon esteso NON venga visualizzata
// 
// ora, per modificare il routing dei messaggi, da ::DefWindowProcW() alla Message Map di CPPTooltipW non
// basta "attaccare" la finestra con this->Attach(hWnd), ma bisogna "subclassarla" con SubclassWindow()
// in modo da scollegare la ::DefWindowProcW() ed agganciare la finestra all'AfxWndProc() di MFC, iniettando
// la Message Map della classe nella finestra Unicode
// 
// in poche parole, Attach(hWnd) associa l'handle ma lascia intatta la procedura della finestra originale
// (utile solo se la finestra e' gia' stata creata da MFC o se non interessano i suoi messaggi), mentre la
// SubclassWindow(hWnd) associa l'handle e devia il flusso dei messaggi dentro la Message Map della classe
// MFC
// 
// basicamente, il subclassing e' una delle tante magie nere di Win32/MFC che, una volta capite, aprono le
// porte a qualsiasi tipo di personalizzazione...
//
#if 0
/////////////////////////////////////////////////////////////////////////////
// CPPToolTip message handlers
//LPI BOOL CPPToolTip::Create(CWnd* pParentWnd, BOOL bBalloon /* = TRUE */) 
BOOL CPPToolTipW::Create(HWND hWndParent, BOOL bBalloon /* = TRUE */) 
{
	TRACE(L"CPPToolTipW::Create\n");
	
//LPI	ASSERT_VALID(pParentWnd);
	
	DWORD dwStyle = WS_POPUP; 
	DWORD dwExStyle = WS_EX_TOOLWINDOW | WS_EX_TOPMOST;
	m_hParentWnd = hWndParent; //LPI pParentWnd->GetSafeHwnd();
	
//LPI	if (!CreateEx(dwExStyle, PPTOOLTIP_CLASSNAME, NULL, dwStyle, 0, 0, 0, 0, pParentWnd->GetSafeHwnd(), NULL, NULL))
	if (!CreateEx(dwExStyle, PPTOOLTIP_CLASSNAME, NULL, dwStyle, 0, 0, 0, 0, hWndParent, NULL, NULL))
		return FALSE;
...
BOOL CPPToolTipW::Create(HWND hWndParent, BOOL bBalloon /* = TRUE */) 
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::Create\n"));
...
BOOL CPPToolTipW::DestroyWindow() 
{
	Pop();
...
/////////////////////////////////////////////////////////////////////
#if _MSC_VER < 1300
void CPPToolTipW::OnActivateApp(BOOL bActive, HTASK hTask)
#else
void CPPToolTipW::OnActivateApp(BOOL bActive, DWORD hTask)
#endif //_MSC_VER
{
...
LRESULT CPPToolTipW::SendNotify(LPPOINT pt, PPTOOLTIP_INFO & ti) 
{ 
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::SendNotify()\n"));
	...
	::SendMessageW(m_hNotifyWnd, WM_NOTIFY, lpnm.hdr.idFrom, (LPARAM)&lpnm);  
...
///////////////////////////////////////////////////////////////////////
BOOL CPPToolTipW::IsNotify()
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::IsNotify\n"));
	
	return (BOOL)(m_hNotifyWnd != NULL);
}  //End of IsNotify

BOOL CPPToolTipW::OnSetCursor(CWnd* /*pWnd*/, UINT /*nHitTest*/, UINT /*message*/) 
{
	CPoint ptClient;
	::GetCursorPos(&ptClient);
	ScreenToClient(&ptClient);
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::OnSetCursor(x=%d, y=%d)\n", ptClient.x, ptClient.y));
...
LRESULT CPPToolTipW::OnRepaintWindow(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::OnRepaintWindow()\n"));
...
void CPPToolTipW::OnDrawBorder(HDC hDC, HRGN hRgn)
{
	ASSERT (hDC);
	ASSERT (hRgn);
...
//
////////////////////////////////////////////////////////////////////////
void CPPToolTipW::OnRedrawTooltip(HDC hDC, BYTE nTransparency /* = 0 */)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::OnRedrawTooltip(Transparency = %d)\n", nTransparency));
...
void CPPToolTipW::OnPaint() 
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::OnPaint()\n"));
...
BOOL CPPToolTipW::PreTranslateMessage(MSG* pMsg) 
{
	RelayEvent(pMsg);
...
BOOL CPPToolTipW::RelayEvent(MSG* pMsg)
{
...
//LPI	ASSERT(m_hParentWnd);
	...
	case WM_SETFOCUS:
		...
		break;
	case WM_LBUTTONDOWN:
		TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::WM_LBUTTONDOWN\n"));
...
//LPI
			...
			{
				// rispedisce il messaggio alla finestra del chiamante
				// puo' passare l'oggetto stesso o l'HWND come WPARAM/LPARAM se serve
				// usa PostMessageW (asincrona) anziche' SendMessage (sincrona) cosi' il tooltip puo' fare la sua Pop()
				// e chiudersi immediatamente, senza attendere che la finestra principale finisca di elaborare il click
				::PostMessageW(m_hWndClickNotify,m_nMsgClickNotify,(WPARAM)this->GetSafeHwnd(),0);
			}
//LPI
		} //if
	...
	case WM_MOUSEMOVE:
		...
		if ((m_ptOriginal.x != pMsg->pt.x) || (m_ptOriginal.y != pMsg->pt.y))
		{
			...
			else
			{
				...
				TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"Временное окно = 0x%08X\n", hWnd));
				if (NULL == hWnd)
				{
				...
				{
					if ((hWnd != m_hwndDisplayedTool) || (ti.rectBounds != m_tiDisplayed.rectBounds))
					{
...
void CPPToolTipW::SetNewTooltip(HWND hWnd, const PPTOOLTIP_INFO & ti, BOOL bDisplayWithDelay /* = TRUE */, TooltipType type /* = PPTOOLTIP_NORMAL */)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::SetNewTooltip(hWnd=0x%08X, CRect(left=%d, top=%d, right=%d, bottom=%d), nID=%d)\n", 
	...
	{
		TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"SetNewTooltip2(%d)\n", m_nNextTooltipType));
...
void CPPToolTipW::OnTimer(UINT nIDEvent) 
{
	POINT pt;
	switch (nIDEvent)
	{
	case TIMER_SHOW:
		TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OnTimerShow(%d)\n", m_nNextTooltipType));
		//ENG: Kill SHOW timer 
		...
		if ((pt.x != m_ptOriginal.x) || (pt.y != m_ptOriginal.y))
		{
			...
			TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OnTimerShow(HideTooltip)\n"));
			HideTooltip();
		}
		else if (PPTOOLTIP_STATE_HIDEN == m_nTooltipState)
		{
			TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OnTimerShow(Showing)\n"));
			...
			{
				TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OnTimerShow(FadeIn)\n"));
				SetTimer(TIMER_SHOWING, m_dwTimeFadeIn, NULL); //Fade in showing was enabled
			}
			else
			{
				TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OnTimerShow(Shown)\n"));
		...
		break;
	case TIMER_SHOWING:
		TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OnTimerFadeIn(Current Transparency=%d )\n", m_dwCurTransparency));
		...
		break;	
	case TIMER_HIDE:
		TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OnTimerHide()\n"));
		...
		break;
	case TIMER_HIDING:
		TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OnTimerFadeOut(Current Transparency=%d)\n", m_dwCurTransparency));
...
void CPPToolTipW::HideTooltip()
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::HideTooltip(CurState=%d)\n", m_nTooltipState));
...
void CPPToolTipW::SetAutoPopTimer()
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::SetAutoPopTimer()\n"));
...
void CPPToolTipW::KillTimers(DWORD dwIdTimer /* = NULL */)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::KillTimers()\n"));
	if (dwIdTimer == NULL)
	{
		KillTimer(TIMER_SHOW);
		KillTimer(TIMER_HIDE);
...
void CPPToolTipW::Pop()
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::Pop()\n"));
	m_nTooltipState = PPTOOLTIP_STATE_HIDEN;
	m_nTooltipType = PPTOOLTIP_NORMAL;
	m_nNextTooltipType = PPTOOLTIP_NORMAL;
	KillTimers();
...
void CPPToolTipW::PrepareDisplayTooltip(LPPOINT lpPoint)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::PrepareDisplayTooltip()\n"));
...
void CPPToolTipW::FreeResources()
{
...
void CPPToolTipW::OutputTooltipOnScreen(LPPOINT lpPoint, HDC /*hDC*/ /* = NULL */)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"OutputTooltipOnScreen()\n"));
	CRect rect = m_rcBoundsTooltip;
...
//
////////////////////////////////////////////////////////////////////
DWORD CPPToolTipW::GetTooltipDirection(DWORD dwDirection, const CPoint & ptPoint, CPoint & ptAnchor, CRect & rcBody, CRect & rcFull, CRect & rcTipArea)
{
	...
	CPPString str;
	str.Format(L"0x%08X", dwDirection);
...
HRGN CPPToolTipW::GetTooltipRgn(DWORD dwDirection, int x, int y, int nWidth, int nHeight)
{
	HRGN hRgn = NULL;
...
BOOL CPPToolTipW::IsCursorOverTooltip() const
{
//LPI    ASSERT(m_hParentWnd);
	...
	CPPToolTipW * pWnd = (CPPToolTipW*)WindowFromPoint(pt);
	
	return (pWnd == this);
}

HWND CPPToolTipW::GetWndFromPoint(const LPPOINT lpPoint, PPTOOLTIP_INFO & ti, BOOL bCheckTool /* = TRUE */)
{
...
BOOL CPPToolTipW::IsComboBoxControl(HWND hWnd, UINT nStyle)
{
	if (hWnd == NULL)
		return FALSE;
	...
	wchar_t szCompare[9];
	::GetClassNameW(hWnd, szCompare, 9);
	return lstrcmpiW(szCompare, L"combobox") == 0;
}

CPPString CPPToolTipW::GetDebugInfoTool(LPPOINT lpPoint)
{
	PPTOOLTIP_INFO ti;
	...
	wchar_t ch[128];
	CPPString str, strTemp;
	CPPString strOutput = L"<table>";
	
	///////////////////////////////////////////////////////////////////
	//Table of a window
	strOutput += L"<tr><td><font color=darkblue>Window</font><table border=1>";

	//1. Window's class name and Window Owner's class name
	::GetClassNameW(hWnd, ch, 128);
	strOutput += CreateDebugCell(L"Class name", ch);
	
	//2. Window's title and Window Owner's title
	::GetWindowTextW(hWnd, ch, 128);
	strOutput += CreateDebugCell(L"Title", ch);
	
	//3. Window's handle and Window Owner's handle
	str.Format(L"0x%08X", hWnd);
	strOutput += CreateDebugCell(L"Handle", str);
	
	//4. Window's ID
	str.Format(L"%d", GetWindowLongW(hWnd, GWL_ID));
	strOutput += CreateDebugCell(L"Control ID", str);

	//5. Window's styles
	str.Format(L"0x%08X", (DWORD)::GetWindowLongW(hWnd, GWL_STYLE));
	strOutput += CreateDebugCell(L"Styles", str);
	
	//6. Window's rect
	RECT rc; 
	::GetWindowRect(hWnd, &rc);
	str.Format(L"(%d, %d)-(%d, %d)", rc.left, rc.top, rc.right, rc.bottom);
	strOutput += CreateDebugCell(L"RECT", str);

	//7. Window's width
	str.Format(L"%d", rc.right - rc.left);
	strOutput += CreateDebugCell(L"Width", str);
	
	//8. Window's height
	str.Format(L"%d", rc.bottom - rc.top);
	strOutput += CreateDebugCell(L"Height", str);

	//9. Window's has tooltip
	HWND hwndTool = FindTool(lpPoint, ti);
	str = (NULL != hwndTool) ? L"Yes" : L"No";
	strOutput += CreateDebugCell(L"Has Tooltip", str);

	strOutput += L"</table></td>";

	///////////////////////////////////////////////////////////////////
	//Table of a window owner
	strOutput += L"<td><font color=darkblue>Window Owner</font><table border=1>";
	
	//1. Window's class name and Window Owner's class name
	if (NULL != hParent)
	{
		::GetClassNameW(hParent, ch, 128);
		str = GetMaxDebugString((CPPString)ch);
	} //if
	else str = L"N/A";
	strOutput += CreateDebugCell(L"Class name", str);
	
	//2. Window's title and Window Owner's title
	if (NULL != hParent)
	{
		::GetWindowTextW(hParent, ch, 128);
		str = GetMaxDebugString((CPPString)ch);
	} //if
	else str = L"N/A";
	strOutput += CreateDebugCell(L"Title", str);
	
	//3. Window's handle and Window Owner's handle
	str.Format(L"0x%08X", hParent);
	strOutput += CreateDebugCell(L"Handle", str);
	...
	///////////////////////////////////////////////////////////////////
	//Table of a window owner
	strOutput += L"<br><font color=darkblue>Mouse Cursor</font><table border=1>";
	
	//1.
	str.Format(L"%d", lpPoint->x);
	strOutput += CreateDebugCell(L"X", str);
	
	//2.
	str.Format(L"%d", lpPoint->y);
	strOutput += CreateDebugCell(L"Y", str);

	strOutput += L"</table></td></tr></table>";

	///////////////////////////////////////////////////////////////////////////
	return strOutput;
}

CPPString CPPToolTipW::CreateDebugCell(CPPString sTitle, LPCWSTR lpszDescription)
{
	CPPString str;
	str.Format(	L"<tr><td width=70 bgcolor=buttonface>%s</td><td width=130 bgcolor=window>%s</td></tr>",
				sTitle.GetString(),
				GetMaxDebugString(lpszDescription).GetString()
				);
	return str;
} //End of CreateDebugCell

CPPString CPPToolTipW::GetMaxDebugString(LPCWSTR lpszText)
{
	CPPString str = (CPPString)lpszText;
	str.Replace(L"<", L"?"); //Replaces the begins of the tags
...
HWND CPPToolTipW::FindTool(const LPPOINT lpPoint, PPTOOLTIP_INFO & ti)
{
	return GetWndFromPoint(lpPoint, ti, TRUE);
} //End of FindTool

BOOL CPPToolTipW::FindTool(HWND hWnd, const LPPOINT lpPoint, PPTOOLTIP_INFO & ti)
{
...
HWND CPPToolTipW::FindToolBarItem(POINT point, PPTOOLTIP_INFO & ti)
{
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::AddTool(CWnd * pWnd, DWORD dwIdString, HICON hIcon, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CPPString str;
	str.LoadString(dwIdString);
	AddTool(pWnd, (LPCWSTR)str, hIcon, lpRectBounds, dwIDTool);
}

void CPPToolTipW::AddTool(CWnd * pWnd, LPCWSTR lpszString, HICON hIcon, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CPPString str;
	str.Format(L"<table><tr><td><icon handle=0x%X></td><td>%s</td></tr></table>", 
		hIcon, lpszString);
	AddTool(pWnd, str, lpRectBounds, dwIDTool);
}

void CPPToolTipW::AddTool(CWnd * pWnd, DWORD dwIdString, DWORD dwIdIcon, CSize szIcon /* = CSize(0, 0) */, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CPPString str;
	str.LoadString(dwIdString);
	AddTool(pWnd, (LPCWSTR)str, dwIdIcon, szIcon, lpRectBounds, dwIDTool);
}

void CPPToolTipW::AddTool(CWnd * pWnd, LPCWSTR lpszString, DWORD dwIdIcon, CSize szIcon /* = CSize(0, 0) */, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CPPString str;
	str.Format(L"<table><tr><td><icon idres=%d width=%d height=%d></td><td>%s</td></tr></table>", 
		dwIdIcon, szIcon.cx, szIcon.cy, lpszString);
	AddTool(pWnd, str, lpRectBounds, dwIDTool);
}

void CPPToolTipW::AddTool(CWnd * pWnd, LPCWSTR lpszString, DWORD dwIdBitmap, COLORREF crMask, CSize szBitmap /*= CSize(0, 0)*/, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CPPString str;
	str.Format(L"<table><tr><td><bmp idres=%d mask=0x%X width=%d height=%d></td><td>%s</td></tr></table>", 
		dwIdBitmap, crMask, szBitmap.cx, szBitmap.cy, lpszString);
	AddTool(pWnd, str, lpRectBounds, dwIDTool);
}

void CPPToolTipW::AddTool(CWnd * pWnd, DWORD dwIdString, LPCRECT lpRectBounds /* = NULL */, DWORD dwIDTool /* = 0 */)
{
	CPPString str;
	str.LoadString(dwIdString);
	AddTool(pWnd, (LPCWSTR)str, lpRectBounds, dwIDTool);
}

void CPPToolTipW::AddTool(CWnd * pWnd, LPCWSTR lpszString /* = NULL */, LPCRECT lpRectBounds /* = NULL */, DWORD dwIDTool /* = 0 */)
{
	PPTOOLTIP_INFO ti;
	...
	ti.sTooltip = (CPPString)lpszString;
	ti.nMask = 0;
	ti.nStyles = 0;
	ti.nDirection = 0;
	ti.nEffect = 0;
...
void CPPToolTipW::AddTool(CWnd * pWnd, PPTOOLTIP_INFO & ti)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::AddTool(hWnd=0x%08X)\n", pWnd->GetSafeHwnd()));
	ASSERT (pWnd);
...
//					      NULL if bounding rectangle don't uses for specified window
////////////////////////////////////////////////////////////////////
void CPPToolTipW::RemoveTool(CWnd * pWnd, LPCRECT lpRectBounds /* = NULL */)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::RemoveTool(hWnd=0x%08X)\n", pWnd->GetSafeHwnd()));
	ASSERT(pWnd);
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::RemoveAllTools()
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::RemoveAllTools()\n"));
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::AddToolBar(CToolBar * pBar)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::AddToolBar(hWnd=0x%08X)\n", pBar->GetSafeHwnd()));
	ASSERT(pBar);
...
BOOL CPPToolTipW::GetToolInfo(PPTOOLTIP_INFO & ti, CWnd * pWnd, LPCRECT lpRectBounds /* = NULL */)
{
	ASSERT(pWnd);
...
BOOL CPPToolTipW::GetToolInfo(PPTOOLTIP_INFO & ti, CWnd * pWnd, DWORD dwIDTool /* = 0 */)
{
	ASSERT(pWnd);
...
void CPPToolTipW::UpdateTipText(LPCWSTR lpszText, CWnd * pWnd, DWORD dwIDTool /* = 0 */)
{
	PPTOOLTIP_INFO ti;
...
void CPPToolTipW::DelTool(CWnd * pWnd, DWORD dwIDTool)
{
	PPTOOLTIP_INFO ti;
	if (GetToolInfo(ti, pWnd, dwIDTool))
	{
...
void CPPToolTipW::SetToolRect(CWnd * pWnd, DWORD dwIDTool, LPCRECT lpRectBounds)
{
	PPTOOLTIP_INFO ti;
	if (GetToolInfo(ti, pWnd, dwIDTool))
	{
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::EnableHyperlink(BOOL bEnable /* = TRUE */)
{
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetCallbackHyperlink(HWND hWnd, UINT nMessage, LPARAM lParam /* = 0 */)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::SetCallbackHyperlink()\n"));
...
//						  Else the notification will not send
///////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetNotify(BOOL bParentNotify /* = TRUE */)
{
	HWND hWnd = NULL;
...
void CPPToolTipW::SetNotify(HWND hWnd)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::SetNotify\n"));
...
//		nValue			- size's value
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetSize(int nSizeIndex, int nValue)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::SetSize(nSizeIndex = %d, nValue = %d)\n", nSizeIndex, nValue));
...
//
/////////////////////////////////////////////////////////////////////////////
int CPPToolTipW::GetSize(int nSizeIndex)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::GetSize(nSizeIndex = %d)\n", nSizeIndex));
	if (nSizeIndex >= PPTTSZ_MAX_SIZES)
		return 0;
...
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetDefaultSizes(BOOL bBalloonSize /* = TRUE */)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::SetDefaultSizes()\n"));
...
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetColorBk(COLORREF color)
{
	SetColorBk(color, color, color);
} //End of SetColorBk

void CPPToolTipW::SetColorBk(COLORREF clrBegin, COLORREF clrEnd)
{
	SetColorBk(clrBegin, clrBegin, clrEnd);
} //End of SetColorBk

void CPPToolTipW::SetColorBk(COLORREF clrBegin, COLORREF clrMid, COLORREF clrEnd)
{
...
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetEffectBk(DWORD dwEffect, BYTE nGranularity /* = 5 */)
{
...
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetBehaviour(DWORD dwBehaviour /* = 0 */)
{
...
//	valid values.
/////////////////////////////////////////////////////////////////////////////
DWORD CPPToolTipW::GetBehaviour()
{
...
/////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetDelayTime(DWORD dwDuration, DWORD dwTime)
{
	switch (dwDuration)
	{
...
///////////////////////////////////////////////////////////////////////
DWORD CPPToolTipW::GetDelayTime(DWORD dwDuration) const
{
	DWORD dwTime = 0;
	switch (dwDuration)
	{
...
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetDirection(DWORD dwDirection /* = PPTOOLTIP_BOTTOMEDGE_LEFT */)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::SetDirection(nDirection = %d)\n", dwDirection));
...
//	the valid values.
/////////////////////////////////////////////////////////////////////////////
DWORD CPPToolTipW::GetDirection()
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::GetDirection()\n"));
...
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetTextStyle(LPCWSTR lpszStyleName, LPCWSTR lpszStyleValue)
{
...
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetCssStyles(LPCWSTR lpszCssStyles /* = NULL */)
{
...
//      lpszPathDll		- 
///////////////////////////////////////////////////////////////////////////// 
void CPPToolTipW::SetCssStyles(DWORD dwIdCssStyle, LPCWSTR lpszPathDll /* = NULL */) 
{ 
...
/////////////////////////////////////////////////////////////////////////////
LPCWSTR CPPToolTipW::GetCssStyles()
{
...
/////////////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetDebugMode(BOOL bDebug /* = TRUE */)
{
	m_bDebugMode = bDebug;
} //End of SetDebugMode
...
//		ti				- Reference to PPTOOLTIP_INFO structure containing the parameters of the tooltip 
////////////////////////////////////////////////////////////////////
void CPPToolTipW::ShowHelpTooltip(LPPOINT pt, DWORD dwIdText, HICON hIcon /* = NULL */)
{
	CPPString str;
	str.LoadString(dwIdText);
	ShowHelpTooltip(pt, (LPCWSTR)str, hIcon);
} //End ShowHelpTooltip

void CPPToolTipW::ShowHelpTooltip(LPPOINT pt, DWORD dwIdText, DWORD dwIdIcon, CSize szIcon /* = CSize(0, 0) */)
{
	CPPString str;
	str.LoadString(dwIdText);
	ShowHelpTooltip(pt, (LPCWSTR)str, dwIdIcon, szIcon);
} //End ShowHelpTooltip

void CPPToolTipW::ShowHelpTooltip(LPPOINT pt, LPCWSTR lpszString, HICON hIcon /* = NULL */)
{
	PPTOOLTIP_INFO ti;
	if (NULL == hIcon)
	{
	...
	{
		ti.sTooltip.Format(L"<table><tr><td><icon handle=0x%X></td><td>%s</td></tr></table>", 
							hIcon, lpszString);
	} //if
...
void CPPToolTipW::ShowHelpTooltip(LPPOINT pt, LPCWSTR lpszString, DWORD dwIdIcon, CSize szIcon /* = CSize(0, 0) */)
{
	CPPString str;
	str.Format(L"<table><tr><td><icon idres=%d width=%d height=%d></td><td>%s</td></tr></table>", 
		dwIdIcon, szIcon.cx, szIcon.cy, lpszString);
	ShowHelpTooltip(pt, (LPCWSTR)str);
} //End ShowHelpTooltip

void CPPToolTipW::ShowHelpTooltip(LPPOINT pt, PPTOOLTIP_INFO & ti)
{
	TRACEEXPR((_TRACE_FLAG_INFO,__FILE__,__LINE__,"CPPToolTipW::ShowHelpTooltip\n"));
...
/*
	ShowHelpTooltipW()
	Versione Wide per icona da ID risorsa.
*/
void CPPToolTipW::ShowHelpTooltipW(LPPOINT pt, LPCWSTR lpcwszString, DWORD dwIdIcon, CSize szIcon /* = CSize(0, 0) */)
{
	if (!lpcwszString)
	{
		ShowHelpTooltip(pt, (LPCWSTR)NULL, dwIdIcon, szIcon);
		return;
	}

	// calcola lo spazio necessario per convertire in ANSI (UTF-8)
	int nLen = ::WideCharToMultiByte(CP_UTF8, 0, lpcwszString, -1, NULL, 0, NULL, NULL);
	if (nLen <= 0) return;

	// alloca il buffer 
	char* pszStringA = (char*)malloc(nLen);
	if (!pszStringA) return;

	// converte da Wide a UTF-8
	::WideCharToMultiByte(CP_UTF8, 0, lpcwszString, -1, pszStringA, nLen, NULL, NULL);

	// passa la chiamata al metodo originale ANSI (LPCWSTR su progetto ANSI e' char*)
	ShowHelpTooltip(pt, (LPCWSTR)pszStringA, dwIdIcon, szIcon);
...
/*
	ShowHelpTooltipW()
	Versione Wide per Icona da HICON handle.
*/
void CPPToolTipW::ShowHelpTooltipW(LPPOINT pt, LPCWSTR lpcwszString, HICON hIcon /* = NULL */)
{
	if (!lpcwszString)
	{
		ShowHelpTooltip(pt, (LPCWSTR)NULL, hIcon);
		return;
	}
	...
	// passa la chiamata al metodo originale ANSI
	ShowHelpTooltip(pt, (LPCWSTR)pszStringA, hIcon);
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetBorder(COLORREF color, int nWidth /* = 1 */, int nHeight /* = 1 */)
{
	HBRUSH hbr = ::CreateSolidBrush(color);
	SetBorder(hbr, nWidth, nHeight);
} //End of SetBorder

void CPPToolTipW::SetBorder(HBRUSH hbr, int nWidth /* = 1 */, int nHeight /* = 1 */)
{
	HideBorder();
...
//
void CPPToolTipW::OnEnterIdle(UINT nWhy, CWnd* pWho)
{
	if ((MSGF_MENU == nWhy))
	{
...
void CPPToolTipW::OnMenuSelect(UINT nItemID, UINT nFlags, HMENU hSubMenu)
{
...
////////////////////////////////////////////////////////////////////
HWND CPPToolTipW::GetRunningMenuWnd()
{
	HWND hwnd = NULL;
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::MenuToolPosition(DWORD nPos /* = PPTOOLTIP_MENU_LEFT | PPTOOLTIP_MENU_TOP */)
{
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::EnableEscapeSequences(BOOL bEnable)
{
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetImageList(UINT nIdBitmap, int cx, int cy, int nCount, COLORREF crMask /* = RGB(255, 0, 255) */)
{
	m_drawer.SetImageList(nIdBitmap, cx, cy, nCount, crMask);
} //End of SetImageList

void CPPToolTipW::SetImageList(HBITMAP hBitmap, int cx, int cy, int nCount, COLORREF crMask /* = RGB(255, 0, 255) */)
{
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetTransparency(BYTE nTransparency /* = 0 */) 
{
	if (nTransparency <= PERCENT_MAX_TRANSPARENCY)
...
////////////////////////////////////////////////////////////////////
void CPPToolTipW::SetTooltipShadow(int nOffsetX, int nOffsetY, BYTE nDarkenPercent /* = 50 */, 
...
void CPPToolTipW::SetImageShadow(int nOffsetX, int nOffsetY, BYTE nDarkenPercent /* = 50 */, 
					BOOL bGradient /* = TRUE */, int nDepthX /* = 7 */, int nDepthY /* = 7 */)
{
